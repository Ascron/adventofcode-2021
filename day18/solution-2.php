...
for ($i = 0, $c = count($pairs); $i < $c; $i++) {
    for ($j = 0; $j < $c; $j++) {
        ...
        $result = new Pair();
        $result->setLeft(readArray($pairs[$i]));
        $result->getLeft()->setParent($result);
        $result->setRight(readArray($pairs[$j]));
        $result->getRight()->setParent($result);
